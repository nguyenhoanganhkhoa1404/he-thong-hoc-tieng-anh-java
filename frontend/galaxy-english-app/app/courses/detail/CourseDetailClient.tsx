...
export default function CourseDetailClient({ id }: { id: string }) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setCourse({
            ...
            duration: "Self-paced",
            lessons: [],
            teacher: { displayName: data.teacherId?.startsWith("T-") ? "Teacher" : data.teacherId }
          });
        }
      } catch (err) {
        console.error(err);
  ...
  const handleEnroll = async () => {
    if (!user || user.role !== "LEARNER") {
      alert("Please login as a Learner to enroll in courses.");
      return;
    }
    setEnrolling(true);
    try {
      // 1. Send Notification to Teacher
      await fetch("/api/admin/notifications", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: "New Student Enrollment! 🎉",
          content: `${user.displayName} has enrolled in your course: ${course.title}.`,
          targetType: "TEACHER",
          targetId: course.teacherId, // We assume teacherId holds the UID of the teacher
          read: false
        })
      });

      // 2. Increment Course Student count
  ...
  if (loading) return <div className="text-white text-center mt-24">Loading course details...</div>;
  if (!course) return <div className="text-white text-center mt-24">Course not found (404)</div>;
        ...
        <div className="relative h-52 rounded-3xl overflow-hidden mb-8 bg-gradient-to-br from-violet-900/60 via-slate-900 to-cyan-900/40 border border-white/10">
          ...
          <div className="absolute bottom-6 left-8 right-8">
            <div className="flex gap-2 mb-2">
              <span className="text-xs px-2 py-0.5 rounded-lg bg-violet-600/30 border border-violet-500/30 text-violet-300">{course.level || "A1"}</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="glass border border-white/10 rounded-2xl p-6">
              ...
              <div className="flex gap-4 mt-4 flex-wrap text-xs text-slate-400">
                <span>📖 {course.moduleIds?.length || 0} lessons</span>
            ...
            <div className="glass border border-white/10 rounded-2xl p-6">
              ...
              {course.lessons.length > 0 ? (
                <div className="space-y-2">
                  {course.lessons.map((lesson: any) => (
                    <div key={lesson.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-colors">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 border border-slate-600 text-slate-400`}>
                        {lesson.order}
                      </div>
                ...
                <p className="text-slate-400 text-sm">Lesson content is being prepared by the Teacher...</p>
              )}
            </div>
          </div>
            ...
            <div className="glass border border-violet-500/30 rounded-2xl p-6 sticky top-24 bg-gradient-to-br from-violet-900/20 to-transparent">
              <div className="text-center mb-5">
                ...
                <p className={`text-3xl font-extrabold ${course.price === 0 ? "text-emerald-400" : "gradient-text"}`}>
                  {course.price === 0 ? "FREE" : `${(course.price || 0).toLocaleString()} VND`}
                </p>
              </div>
              <Button variant="neon" size="lg" className="w-full mb-3" onClick={handleEnroll} disabled={enrolling || enrolled}>
                {enrolled ? "✅ Enrolled Successfully!" : enrolling ? "Enrolling..." : course.price === 0 ? "🚀 Enroll Free" : "💳 Enroll Now"}
              </Button>
